...
module UNRParkingZonesLogic
  def find_available_zones
    if @pass == 'Blue'
      ...
      @zones = [public_send(designated_zone)]
      add_green_and_blue
      add_tan
    elsif @pass == 'Silver'
      find_silver_zones
  ...
  def find_evening_zones
    if (1530..1730).cover?(@time)
      @zones = [silver17[0]]
    elsif (1730..2400).cover?(@time) || (0..700).cover?(@time)
      @zones = []
      add_silver
      add_tan
      add_green_and_blue
  ...
  def find_silver_zones
    if (0..700).cover?(@time) || (1730..2400).cover?(@time)
      @zones = []
      add_silver
    else
  ...
  def find_east_campus_zones
    if (0..700).cover?(@time) || (1530..2400).cover?(@time)
      @zones = [silver17[0]]
      add_green_and_blue
      add_tan
    else
